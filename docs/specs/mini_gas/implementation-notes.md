...
3. **接口化解耦**：通过 `IEntityModule`、`IWorldModule`、`IDebug`、`ApplyFun` 与业务状态解耦，便于集成到现有系统。
...
5. **ModifierAttributeEval 参数**：首次调用时固定传入 `id = nil, value = nil`，随后才是当前 Ability 产生的 `modifier_args`。`modifier_args` 来源：对象形式为 `{ count, ... }`；函数形式为函数返回的 `...`；为空时直接采用 `ASC.evaluate` 调用者传入的上下文参数。
6. **数值稳定**：Modifier 聚合顺序固定：先累加 Add，再连乘 Multiply，最后判断 Override；最终值按 `AttributeDef.min/max` 截断。
7. **单一 ApplyFun 与 target 级应用**：每个 `target` 实体在全部求值完成后，只调用一次 `ApplyFun`，传递聚合后的标签集合 `tags`（`table<mini_gas.Tag, boolean>`）与 `attributes` 映射（`table<mini_gas.ID, number>`）。Add 语义下 `attributes[id] = new_value - base`，业务方通过“旧值 + 差值”得到最终值。
8. **对象池与所有权**：`asc.lua` 内部使用模块级对象池复用求值过程中产生的所有临时表（`evaluate_args`、`modifier_args`、`tags`、`attributes`、`deltas` 及其嵌套表等），降低 GC 压力。
   - `ApplyFun` 收到的 `tags` 与 `attributes` 归库所有，`apply` 返回后会被立即回收。
   - 对象池实现带有重复释放保护，避免同一张表在池中出现多次。

...
17. **配置与状态无元表**：所有配置对象（Spec / Def）、运行时状态（EntityState / WorldState）以及运行时数据对象（Modifier / GameplayEffect / GameplayAbility / GameplayTagContainer / GameplayTask）的实例均使用无元表的普通 Lua 表，便于外部配置桥接、序列化与持久化；相关操作通过对应模块的函数完成。运行时属性值仅存储为 `EntityState.attributes` 中的普通数字，不再使用 Attribute 对象。
...
| v2.0 | 2026-06-14 | 重构为独立 GAS 核心；新增 Ability / Tag / Spec；目录调整为 `lua_lib/mini_gas`；业务 ID 由策划 alias（`string \| integer`）配置；`MiniASC` 改为无状态函数集合，状态由 `EntityState` / `WorldState` 外置；运行时实例自包含完整 Def 数据；配置定义由 `Defs` 持有并传入；效果通过 Granted / Require / Block Tag 驱动 |

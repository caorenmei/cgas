...
- **Current 读取值**：`MiniASC.get_current(state, attr_id)` 在已提交值基础上叠加生效的持续 Modifier，并 Clamp 到边界。

#### 6.2.2 成长曲线

Attribute 的值可通过 `GrowthCurve` 随等级成长。`GrowthCurve` 是任意公式函数 `fun(level: number, ...): number`，不强制 `base` / `params` 字段；禁止通过等级查表实现。
...
Modifier 的 `value` 类型为 `number`（用于 `Add` / `Multiply` / `Override`）或 `fun(self: Modifier, v: number): number`（仅用于 `Compound`）。若需要按等级成长，应由 `ConfigAdapter` 在 `apply_effect` 前按目标等级生成对应的 `number`。

...
- 遍历 `active_abilities`，将每个 `modifier_args` 清空后放回 `short_array_pool`。
- 清空 `active_abilities` 本身，放回 `long_array_pool`。
- 清空 `evaluate_args`，放回 `short_array_pool`。

#### 对象池分类

库内部保留三类对象池：

- `table_pool`：复用键值型临时表，回收时通过 `pairs` 遍历将所有键值置 `nil`。用于 `tags`、`attributes`、`deltas` 等键值映射。
- `short_array_pool`：复用短数组型临时表，回收时执行 `local n = t.n; for i = 1, n do t[i] = false end; t.n = 0`。用于 `evaluate_args`、`modifier_args`、`pairs_list` 等生命周期较短或元素较少的数组。
- `long_array_pool`：复用长数组型临时表，回收语义与 `short_array_pool` 相同，专门用于生命周期跨越整个求值流程且元素较多的 `active_abilities`。

> 注意：数组池的表以 `.n` 字段作为长度标准，不使用 `#` 或 `ipairs`，因为回收时数组元素被置为 `false` 而非 `nil`，`#` 会把末尾残留的 `false` 计入长度。

所有对象池均带有重复释放保护（通过 `__in_pool` 标记），避免同一张表在池中出现多次。

#### 属性聚合内部结构

`apply_to_targets` 中每个目标实体的 `attributes` 是一张以 `attr_id` 为键的映射表，其值 `attr_entry` 采用数组结构以进一步减少临时表字段：

```
attr_entry[1] = override_value  -- Override 的最终值；false 表示未覆盖
attr_entry[2] = add_sum         -- 所有 Add 值的累加和
attr_entry[3] = multiply_product -- 所有 Multiply 值的连乘积
```

数组池中的表避免使用 `nil` 作为占位值，未设置的 Override 位置用 `false` 填充，避免数组出现空洞导致 `#` / `ipairs` 行为异常。

最终值计算规则：

- 若 `attr_entry[1] ~= false`，则 `final = attr_entry[1]`；
- 否则 `final = (base + attr_entry[2]) * attr_entry[3]`。

得到 `final` 后按 `Defs.attribute_defs[id]` 的 `min` / `max` 截断，并记录 `attributes[id] = final - base`。

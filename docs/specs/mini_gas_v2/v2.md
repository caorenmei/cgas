...
- `table_pool`：复用键值型临时表，回收时通过 `pairs` 遍历将所有键值置 `nil`。用于 `tags`、`attributes`、`deltas`、`attr_entry`、`modifier_args` 等。
- `array_pool`：复用纯数组型临时表，回收时执行 `local n = t.n; for i = 1, n do t[i] = false end; t.n = 0`。用于 `evaluate_args`、`active_abilities`、`pairs_list` 及其元素等。

> 注意：`array_pool` 的表以 `.n` 字段作为长度标准，不使用 `#` 或 `ipairs`，因为回收时数组元素被置为 `false` 而非 `nil`，`#` 会把末尾残留的 `false` 计入长度。

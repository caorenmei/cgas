...
---@field begin_ability? fun(context: mini_gas.IContext, owner_id: mini_gas.ID, owner_entity: mini_gas.IEntityState, owner_module: mini_gas.IEntityModule, ability_def_id: mini_gas.ID, ...: unknown) 开始评估某个 Ability
---@field end_ability? fun(context: mini_gas.IContext, owner_id: mini_gas.ID, owner_entity: mini_gas.IEntityState, owner_module: mini_gas.IEntityModule, ability_def_id: mini_gas.ID, ...: unknown) 结束评估某个 Ability
---@field begin_effect? fun(context: mini_gas.IContext, owner_id: mini_gas.ID, owner_entity: mini_gas.IEntityState, owner_module: mini_gas.IEntityModule, ability_def_id: mini_gas.ID, effect_def_id: mini_gas.ID, ...: unknown) 开始应用某个 Effect
---@field end_effect? fun(context: mini_gas.IContext, owner_id: mini_gas.ID, owner_entity: mini_gas.IEntityState, owner_module: mini_gas.IEntityModule, ability_def_id: mini_gas.ID, effect_def_id: mini_gas.ID, ...: unknown) 结束应用某个 Effect
---@field begin_modifier? fun(context: mini_gas.IContext, owner_id: mini_gas.ID, owner_entity: mini_gas.IEntityState, owner_module: mini_gas.IEntityModule, ability_def_id: mini_gas.ID, effect_def_id: mini_gas.ID, modifier_def: mini_gas.ModifierDef, target_entity: mini_gas.IEntityState, target_module: mini_gas.IEntityModule, ...: unknown) 开始评估某个 Modifier
---@field end_modifier? fun(context: mini_gas.IContext, owner_id: mini_gas.ID, owner_entity: mini_gas.IEntityState, owner_module: mini_gas.IEntityModule, ability_def_id: mini_gas.ID, effect_def_id: mini_gas.ID, modifier_def: mini_gas.ModifierDef, target_entity: mini_gas.IEntityState, target_module: mini_gas.IEntityModule, ...: unknown) 结束评估某个 Modifier
...
---@param context mini_gas.IContext 系统上下文，由库的使用者实现，包含 world、world_module、defs 以及业务方自定义字段
---@param debug? mini_gas.IDebug 调试/追踪接口，可选；不需要时传 nil
...
3. 将 `world`、`world_module`、`defs` 以及业务方自定义字段组装进 `context`。
4. 调用 `ASC.evaluate(context, debug, apply, ...)` 进行全量求值；其中 `debug` 为可选调试接口，不需要时可传 `nil`。
...
```mermaid
flowchart TD
    A["调用 ASC.evaluate(context, debug, apply)"] --> B["阶段一：收集激活能力"]
...
- 若提供了 `IDebug`，库会在合适的时机调用 `begin_ability` / `end_ability` / `begin_effect` / `end_effect` / `begin_modifier` / `end_modifier` / `step` 等钩子。

### 求值实现：两阶段流程与对象池

`ASC.evaluate` 的实际实现采用“先收集、再应用、最后回收”的两阶段流程，并通过对象池复用中间表，避免每次求值产生大量临时 Lua 表。
...
- 遍历 `active_abilities`，将每个 `modifier_args` 清空后放回 `modifier_args_pool`。
- 清空 `active_abilities` 本身，放回 `ability_list_pool`。
...
local ability_list_pool = {}
local modifier_args_pool = {}
...
local function pop(pool)
    ...
    if t then return clear_table(t) end
    return {}
end

local function push(pool, t)
    clear_table(t)
    pool[#pool + 1] = t
end

-- 阶段一：收集激活能力
local active_abilities = pop(ability_list_pool)
for owner_id, owner_entity, owner_module in context.world_module.entities(context) do
    for ability_id in owner_module.static_abilities(owner_entity) do
        local ability_def = context.defs.ability_defs[ability_id]
        local modifier_args = pop(modifier_args_pool)
        local activated = check_can_activate(context, owner_entity, owner_module, ability_def, modifier_args)
        if activated then
            ...
            push(modifier_args_pool, modifier_args)
        end
    end
end

-- 阶段二：逐实体应用
for target_id, target_entity, target_module in context.world_module.entities(context) do
    local tags = pop(tag_pool)
    local attributes = pop(attribute_pool)
    ...
    for i = 1, #active_abilities, 3 do
        ...
        local ability_def = context.defs.ability_defs[ability_id]
        for _, effect_id in ipairs(ability_def.effects) do
            local effect_def = context.defs.effect_defs[effect_id]
            if match_effect_target(context, owner_id, target_id, target_entity, target_module, effect_def) then
                for _, tag in ipairs(effect_def.grant_tags or {}) do
                    tags[tag] = true
                end

                for _, modifier_def in ipairs(effect_def.modifiers) do
                    if match_modifier_tags(context, target_entity, target_module, modifier_def) then
                        resolve_modifier(context, target_entity, target_module, modifier_def, modifier_args, attributes)
                    end
                end
            end
        end
    ...
    clamp_attributes(context, target_entity, target_module, attributes)
    apply(context, target_entity, tags, attributes, ...)

    push(tag_pool, tags)
    push(attribute_pool, attributes)
end

-- 阶段三：回收
for i = 3, #active_abilities, 3 do
    push(modifier_args_pool, active_abilities[i])
end
push(ability_list_pool, active_abilities)
```
